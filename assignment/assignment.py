...
def generate_sample(die_type_counts: Tuple[int],
    ...
    np.random.seed(seed)
    die_chosen_array: np.ndarray = np.random.choice(
        a = die_type_counts_array,
        size = num_draws,
        p = die_type_probs
    )
    die_types_drawn = np.concatenate(
        [
            np.flatnonzero(die_type_counts_array == die_prob) for die_prob in die_chosen_array[
                np.where(
                    np.isin(die_chosen_array, die_type_counts_array)
                )
            ]
        ]
    )
